...
class TestMem0Metrics(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            self.mock_meter.create_histogram.call_count, 4
        )  # duration histograms
        self.assertEqual(
            self.mock_meter.create_counter.call_count, 12
        )  # count, error, slow counters
    ...
    def test_metric_names_from_semconv(self):
        ...
        self.assertIn(
            SemanticAttributes.METRIC_OPERATION_DURATION, histogram_names
        )
        self.assertIn(
            SemanticAttributes.METRIC_VECTOR_OPERATION_DURATION,
            histogram_names,
        )
        self.assertIn(
            SemanticAttributes.METRIC_GRAPH_OPERATION_DURATION, histogram_names
        )
        self.assertIn(
            SemanticAttributes.METRIC_RERANKER_OPERATION_DURATION,
            histogram_names,
        )
...
class TestMetricsRecording(unittest.TestCase):
    ...
    def test_memory_add_operation_metrics(self):
        ...
        self.assertEqual(
            metric_attrs[self.SemanticAttributes.METRIC_GEN_AI_OPERATION_NAME],
            "memory_operation",
        )
        self.assertEqual(
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_OPERATION
            ],
            "add",
        )
    ...
    def test_vector_subphase_metrics_with_provider_and_url(self):
        ...
        if self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_PROVIDER in span_attrs:
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_PROVIDER
            ] = span_attrs[
                self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_PROVIDER
            ]
        if self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_URL in span_attrs:
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_URL
        ...
        self.assertEqual(
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_METHOD
            ],
            "search",
        )
        self.assertEqual(
    ...
    def test_graph_subphase_metrics_with_provider_and_url(self):
        ...
        if self.SemanticAttributes.GEN_AI_MEMORY_GRAPH_PROVIDER in span_attrs:
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_GRAPH_PROVIDER
            ] = span_attrs[
                self.SemanticAttributes.GEN_AI_MEMORY_GRAPH_PROVIDER
            ]
        if self.SemanticAttributes.GEN_AI_MEMORY_GRAPH_URL in span_attrs:
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_GRAPH_URL
        ...
        self.assertEqual(
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_GRAPH_METHOD
            ],
            "add",
        )
        self.assertEqual(
    ...
    def test_reranker_subphase_metrics_with_provider(self):
        ...
        if (
            self.SemanticAttributes.GEN_AI_MEMORY_RERANKER_PROVIDER
            in span_attrs
        ):
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_RERANKER_PROVIDER
            ] = span_attrs[
                self.SemanticAttributes.GEN_AI_MEMORY_RERANKER_PROVIDER
            ]
        ...
        self.assertEqual(
            metric_attrs[
                self.SemanticAttributes.METRIC_GEN_AI_MEMORY_RERANKER_METHOD
            ],
            "rerank",
        )
        self.assertEqual(
    ...
    def test_error_count_metric_with_error_type(self):
        ...
        self.assertEqual(
            error_attrs[self.SemanticAttributes.METRIC_ERROR_TYPE],
            "ValueError",
        )
    ...
    def test_metric_attribute_name_format(self):
        ...
        self.assertEqual(
            self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_METHOD,
            "gen_ai_memory_vector_method",
        )
        self.assertEqual(
            self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_PROVIDER,
            "gen_ai_memory_vector_provider",
        )
        self.assertEqual(
            self.SemanticAttributes.METRIC_GEN_AI_MEMORY_VECTOR_URL,
            "gen_ai_memory_vector_url",
        )

        # Trace attribute names should use dot notation
        self.assertEqual(
            self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_METHOD,
            "gen_ai.memory.vector.method",
        )
        self.assertEqual(
            self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_PROVIDER,
            "gen_ai.memory.vector.provider",
        )
        self.assertEqual(
            self.SemanticAttributes.GEN_AI_MEMORY_VECTOR_URL,
            "gen_ai.memory.vector.url",
        )

    def test_memory_metrics_should_not_include_high_cardinality_attributes(
        self,
    ):
        ...
        self.assertIn(
            self.SemanticAttributes.METRIC_GEN_AI_OPERATION_NAME, metric_attrs
        )
        self.assertIn(
            self.SemanticAttributes.METRIC_GEN_AI_MEMORY_OPERATION,
            metric_attrs,
        ...
        self.assertNotIn(
            self.SemanticAttributes.GEN_AI_MEMORY_USER_ID, metric_attrs
        )
        self.assertNotIn(
            self.SemanticAttributes.GEN_AI_MEMORY_AGENT_ID, metric_attrs
        )
        self.assertNotIn(
            self.SemanticAttributes.GEN_AI_MEMORY_RUN_ID, metric_attrs
        )
        self.assertNotIn(
            self.SemanticAttributes.GEN_AI_MEMORY_APP_ID, metric_attrs
        )
    ...
    def test_memory_client_metrics_should_include_server_info(self):
        ...
        self.assertIn(
            self.SemanticAttributes.METRIC_SERVER_ADDRESS, metric_attrs
        )
