...
from ..shared import CommonAttributes, GenAiSpanKind, ToolRequestAttributes
from .utils import (
...
def toolkit_call_tool_function(
    ...
    async def traced_method(wrapped, instance, args, kwargs) -> ToolResponse:
        ...
        # 创建tool请求属性
        request_attrs = ToolRequestAttributes(
        ...
        if isinstance(tool_call, dict):  # type: ignore
            ...
            request_attrs.tool_call_arguments = _serialize_to_str(
                tool_call.get("input")
            )
            request_attrs.tool_description = _get_tool_description(
                instance=instance, tool_name=request_attrs.tool_name
            )
