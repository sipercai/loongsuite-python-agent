...
try:
    ...
    _logger.warning("numpy or soundfile not available, PCM16 to WAV conversion will be skipped")
...
class MultimodalPreUploader(PreUploader):
    ...
    def __init__(self, base_path: str, extra_meta: Optional[Dict[str, str]] = None) -> None:
        self._base_path = base_path
        self._extra_meta = extra_meta or {}
        
        # 读取多模态上传配置（静态配置，只需读取一次）
        upload_mode = os.getenv(OTEL_INSTRUMENTATION_GENAI_MULTIMODAL_UPLOAD_MODE, "both").lower()
    ...
    def _ensure_loop(cls) -> asyncio.AbstractEventLoop:
        """确保事件循环存在且正在运行（线程安全）"""
        # 快速路径：循环存在且线程存活
        if cls._loop is not None and cls._loop_thread is not None and cls._loop_thread.is_alive():
            return cls._loop
    ...
    def _at_fork_reinit(cls) -> None:
        """Fork 后在子进程中重置类级别状态"""
        _logger.debug("[_at_fork_reinit] MultimodalPreUploader reinitializing after fork")
    ...
    def _run_async(self, coro, timeout: float = 0.3):
        """在类级别事件循环中执行协程（线程安全）"""
        cls = self.__class__
        ...
        with suppress_http_instrumentation():
            try:
                async with httpx.AsyncClient(
                    ...
                    tasks = [self._fetch_one_metadata_async(client, uri) for uri in uris]
    ...
    def _detect_audio_format(data: bytes) -> Optional[str]:
        ...
        if len(data) >= 12:
            if data[4:8] == b'ftyp':
                ...
                # 3GP 格式
                if ftyp_brand == b'3gp' or ftyp_brand == b'3gr' or ftyp_brand == b'3gs':
                    return "audio/3gpp"
                # 3GP2 格式
                if ftyp_brand == b'3g2':
                    return "audio/3gpp2"
        ...
        if len(data) >= 8 and data[4:8] == b'ftyp':
            ...
            # M4A 格式
            if ftyp_brand in (b'M4A ', b'M4B ', b'M4P ', b'M4V ', b'mp42', b'isom'):
                return "audio/m4a"
    ...
    @staticmethod
    def _convert_pcm16_to_wav(pcm_data: bytes, sample_rate: int = 24000) -> Optional[bytes]:
        """
        ...
        if not _AUDIO_LIBS_AVAILABLE:
            _logger.warning("Cannot convert PCM16 to WAV: numpy or soundfile not available")
            return None
    ...
    def _create_download_upload_item(
        ...
        uri_part = Uri(modality=modality, mime_type=metadata.content_type, uri=full_url)
        return upload_item, uri_part
    ...
    def _process_message_parts(
        ...
        for i, part in enumerate(parts):
            if len(blob_parts) + len(uri_parts) >= _MAX_MULTIMODAL_PARTS:
                _logger.debug(f"Reached max multimodal parts limit ({_MAX_MULTIMODAL_PARTS}), skipping remaining")
                break
        ...
        for i, part in blob_parts:
            try:
                ...
                if len(data) > _MAX_MULTIMODAL_DATA_SIZE:
                    _logger.debug(f"Skip Blob: size {len(data)} exceeds limit {_MAX_MULTIMODAL_DATA_SIZE}")
                    continue
                ...
                if mime_type in ("audio/unknown", "audio/*", "audio"):
                    ...
                    if detected_mime:
                        _logger.info("Auto-detected audio format: %s -> %s", mime_type, detected_mime)
                        mime_type = detected_mime
                # 如果是 PCM16 音频格式，转换为 WAV
                if mime_type in ("audio/pcm16", "audio/l16", "audio/pcm"):
                    wav_data = self._convert_pcm16_to_wav(data)
                    if wav_data:
                        _logger.info("Converted PCM16 to WAV format, original size: %d, new size: %d", len(data), len(wav_data))
                        mime_type = "audio/wav"
                        data = wav_data
                    else:
                        _logger.warning("Failed to convert PCM16 to WAV, using original format")
                
                upload_item, uri_part = self._create_upload_item(
                    data, mime_type, part.modality, timestamp, trace_id, span_id
                )
                uploads.append(upload_item)
                parts[i] = uri_part
            except Exception as e:
                _logger.error(f"Failed to process Blob/Blob, skip: {e}, trace_id: {trace_id}")
                # 保持原样，不替换
        ...
        for i, part in uri_parts:
            ...
            if not self._is_http_uri(part.uri):
                _logger.debug(f"Skip non-http URI (already processed or local): {part.uri}")
                continue
            
            metadata = uri_to_metadata.get(part.uri)
            # 获取失败/超时/缺失必要信息 -> 保持原样
            if metadata is None:
                _logger.debug(f"No metadata for URI (timeout/error/missing), skip: {part.uri}")
                continue
            
            # 大小限制检查
            if metadata.content_length > _MAX_MULTIMODAL_DATA_SIZE:
                _logger.debug(f"Skip Uri: size {metadata.content_length} exceeds limit {_MAX_MULTIMODAL_DATA_SIZE}, uri: {part.uri}")
                continue
            
            try:
                upload_item, uri_part = self._create_download_upload_item(
                    part.uri, metadata, part.modality, timestamp, trace_id, span_id
                )
                uploads.append(upload_item)
                parts[i] = uri_part
                _logger.debug(f"Uri processed: {part.uri} -> {uri_part.uri}, expected_size: {metadata.content_length}")
            except Exception as e:
                _logger.error(f"Failed to process Uri, skip: {e}, uri: {part.uri}")
                # 保持原样，不替换
            ...
            for msg in input_messages:
                if hasattr(msg, 'parts') and msg.parts:
                    self._process_message_parts(
                        msg.parts, trace_id, span_id, timestamp, uri_to_metadata, uploads
                    )
