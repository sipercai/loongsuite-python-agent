...
from typing import TYPE_CHECKING, Any, ClassVar, Literal, Optional, Tuple
...
from opentelemetry.util.genai.types import Blob, Error, LLMInvocation, Uri
...
class MultimodalProcessingMixin:
    ...
    # 类级别共享的异步处理资源
    _async_queue: ClassVar[Optional[queue.Queue]] = None
    ...
    def _init_multimodal(self) -> None:
        ...
        self._multimodal_enabled = upload_mode != "none" and uploader is not None and pre_uploader is not None
    ...
    def process_multimodal_stop(self, invocation: LLMInvocation) -> bool:
        ...
        # 3. 尝试放入队列（非阻塞）
        try:
            self._async_queue.put_nowait(_MultimodalAsyncTask(
            ...
            # 队列满：同步降级，跳过多模态处理
            _logger.warning("Multimodal queue full, skipping multimodal processing")
    ...
    def _quick_has_multimodal(self, invocation: LLMInvocation) -> bool:
        ...
        def _check_messages(messages) -> bool:
            if not messages:
                return False
            for msg in messages:
        ...
        return (_check_messages(invocation.input_messages) or 
                _check_messages(invocation.output_messages))
    ...
    def _async_worker_loop(cls) -> None:
        ...
        while True:
            ...
            try:
                if task.method == 'stop':
                    task.handler._async_stop_llm(task)
                elif task.method == 'fail':
                    task.handler._async_fail_llm(task)
            except Exception as e:
                _logger.warning("Multimodal async processing error: %s", e)
                # 确保 span 被结束
                try:
                    end_time_ns = task.handler._compute_end_time_ns(task.invocation)
                    task.invocation.span.end(end_time=end_time_ns)
                except Exception:
                    pass
            finally:
                # 使用局部变量避免竞态条件
    ...
    def _async_stop_llm(self, task: _MultimodalAsyncTask) -> None:
        ...
        if uploader is not None and pre_uploader is not None:
            ...
            # 提取并设置多模态元数据
            input_metadata, output_metadata = self._extract_multimodal_metadata(
                invocation.input_messages, invocation.output_messages
            )
            if input_metadata:
                span.set_attribute(
                    GenAI.GEN_AI_INPUT_MULTIMODAL_METADATA,
                    json.dumps(input_metadata)
                )
            if output_metadata:
                span.set_attribute(
                    GenAI.GEN_AI_OUTPUT_MULTIMODAL_METADATA,
    ...
    def _async_fail_llm(self, task: _MultimodalAsyncTask) -> None:
        ...
        # 3. 记录指标
        error_type = getattr(error.type, "__qualname__", None) if error else None
    ...
    def _fallback_fail_span(self, invocation: LLMInvocation, error: Error) -> None:
    ...
    def _extract_multimodal_metadata(
        ...
        """从消息中提取多模态元数据"""
        def _extract_from_messages(messages) -> list:
            metadata = []
            if not messages:
                return metadata
            for msg in messages:
...
if hasattr(os, "register_at_fork"):
    os.register_at_fork(after_in_child=MultimodalProcessingMixin._at_fork_reinit)
